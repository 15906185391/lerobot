...
class LCMHandler:
    ...
    def manip_lcm_handle(self):
        while not self._stop_event.is_set():
            try:
                if hasattr(self.manip_lcm, "handle_timeout"):
                    self.manip_lcm.handle_timeout(100)
                else:
                    self.manip_lcm.handle()
            except Exception as exc:
                logger.debug("LCM handler loop error: %s", exc)

    def stop(self):
        self._stop_event.set()
        if self.manip_lcm_thread_handle.is_alive():
            self.manip_lcm_thread_handle.join(timeout=0.5)
    ...
    def manip_waist_state_listener(self, channel, data):
        try:
            ...
            logger.debug("Failed to decode waist state: %s", exc)
    ...
    def manip_leg_state_listener(self, channel, data):
        try:
            ...
            logger.debug("Failed to decode leg state: %s", exc)
