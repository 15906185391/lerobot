...
def send_hardware_upper_body_command(
    handler, arm_q: np.ndarray, waist_q: np.ndarray, enabled: bool
) -> None:
    ...
    package[18:21] = np.asarray(waist_q, dtype=float)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Send left/right arm and torso joint commands through "
    ...
    waist_q_indices, _ = waist_joint_indices(robot.model)
        ...
        while True:
            ...
            if hardware_interface is not None:
                send_hardware_upper_body_command(
                    ...
                    configuration.q[waist_q_indices],
