...
def convert_dataset(
    ...
    if delete_old_stats and (dataset.root / LEGACY_EPISODES_STATS_PATH).is_file():
        (dataset.root / LEGACY_EPISODES_STATS_PATH).unlink()

    hub_api = HfApi()
    if delete_old_stats and hub_api.file_exists(
        repo_id=dataset.repo_id, filename=LEGACY_EPISODES_STATS_PATH, revision=branch, repo_type="dataset"
    ):
        hub_api.delete_file(
            path_in_repo=LEGACY_EPISODES_STATS_PATH, repo_id=dataset.repo_id, revision=branch, repo_type="dataset"
        )
    if push_to_hub:
