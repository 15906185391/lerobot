...
def main():
    ...
    if _LEGACY_IMPORT_ERROR is not None:
        raise SystemExit(
            "LeRobot v1.6 -> v2.0 conversion requires an older LeRobot environment that still provides "
            "`lerobot.common.*` modules. Please run this converter in a matching legacy environment, or "
            f"use newer version converters in this GUI. Original import error: {_LEGACY_IMPORT_ERROR}"
        )
